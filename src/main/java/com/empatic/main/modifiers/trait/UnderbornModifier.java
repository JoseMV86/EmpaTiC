...
public class UnderbornModifier extends Modifier implements MeleeDamageModifierHook {
  ...
  private static final float BOOST_AT_0 = 0.1f;
   ...
   public float getMeleeDamage(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float baseDamage, float damage) {
    ...
    float factor = Math.max(1f, (SEA_LEVEL - (float) context.getTarget().getY()) * modifier.getEffectiveLevel() * (BOOST_AT_0 / SEA_LEVEL) + 1);
    if (factor > 1f) {
